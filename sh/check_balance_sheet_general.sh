...
usage(){
    echo "usage:"
    echo "check_balance_sheet_general.sh <file_list>"
    exit 1
}
...
jqf="$DIL_ROOT"/tmp/check_balance_sheet_general.jq
rm -rf "$jqf"

"$DIL_ROOT"/sh/print_regular_report_delta_jq_fields.sh -c bs general > "$jqf"
