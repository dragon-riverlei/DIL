...
schema_lst_bank="$DIL_ROOT"/feeds/fdmt_balance_sheet_initial/schemas/000001.lst
jqf_bank="$DIL_ROOT"/tmp/extract_balance_sheet_bank.jq
"$DIL_ROOT"/sh/print_regular_report_delta_jq_fields.sh -e bs bank > "$jqf_bank"
...
schema_lst_general="$DIL_ROOT"/feeds/fdmt_balance_sheet_initial/schemas/000002.lst
jqf_general="$DIL_ROOT"/tmp/extract_balance_sheet_general.jq
"$DIL_ROOT"/sh/print_regular_report_delta_jq_fields.sh -e bs general > "$jqf_general"
table=securities_balance_sheet_general
for code in $(cat "$schema_lst_general" <(echo "$3" | tr "," "\n") | sort | uniq -d)
do
    "$DIL_ROOT"/sh/check_balance_sheet_general.sh "$DIL_ROOT"/feeds/fdmt_balance_sheet_delta/data/"$code"_"$1".jl
    jl_file="$DIL_ROOT"/feeds/fdmt_balance_sheet_delta/data/"$code"_"$1".jl
    csv_file="$DIL_ROOT"/feeds/fdmt_balance_sheet_delta/dbdata/"$code"_$1.csv
...
schema_lst_insurance="$DIL_ROOT"/feeds/fdmt_balance_sheet_initial/schemas/601318.lst
jqf_insurance="$DIL_ROOT"/tmp/extract_balance_sheet_insurance.jq
"$DIL_ROOT"/sh/print_regular_report_delta_jq_fields.sh -e bs insurance > "$jqf_insurance"
table=securities_balance_sheet_insurance
for code in $(cat "$schema_lst_insurance" <(echo "$3" | tr "," "\n") | sort | uniq -d)
do
    "$DIL_ROOT"/sh/check_balance_sheet_insurance.sh "$DIL_ROOT"/feeds/fdmt_balance_sheet_delta/data/"$code"_"$1".jl
    jl_file="$DIL_ROOT"/feeds/fdmt_balance_sheet_delta/data/"$code"_"$1".jl
    csv_file="$DIL_ROOT"/feeds/fdmt_balance_sheet_delta/dbdata/"$code"_$1.csv
...
schema_lst_securities="$DIL_ROOT"/feeds/fdmt_balance_sheet_initial/schemas/000686.lst
jqf_securities="$DIL_ROOT"/tmp/extract_balance_sheet_securities.jq
"$DIL_ROOT"/sh/print_regular_report_delta_jq_fields.sh -e bs securities > "$jqf_securities"
table=securities_balance_sheet_securities
for code in $(cat "$schema_lst_securities" <(echo "$3" | tr "," "\n") | sort | uniq -d)
do
    "$DIL_ROOT"/sh/check_balance_sheet_securities.sh "$DIL_ROOT"/feeds/fdmt_balance_sheet_delta/data/"$code"_"$1".jl
    jl_file="$DIL_ROOT"/feeds/fdmt_balance_sheet_delta/data/"$code"_"$1".jl
    csv_file="$DIL_ROOT"/feeds/fdmt_balance_sheet_delta/dbdata/"$code"_$1.csv
