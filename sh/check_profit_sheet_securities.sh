...
usage(){
    echo "usage:"
    echo "check_profit_sheet_securities.sh <file_list>"
    exit 1
}
...
jqf="$DIL_ROOT"/tmp/check_profit_sheet_securities.jq
rm -rf "$jqf"

"$DIL_ROOT"/sh/print_regular_report_delta_jq_fields.sh -c ps securities > "$jqf"
