...
usage(){
    echo "usage:"
    echo "check_cash_flow_sheet_bank.sh <file_list>"
    exit 1
}
...
jqf="$DIL_ROOT"/tmp/check_cash_flow_sheet_bank.jq
rm -rf "$jqf"

"$DIL_ROOT"/sh/print_regular_report_delta_jq_fields.sh -c cfs bank > "$jqf"
