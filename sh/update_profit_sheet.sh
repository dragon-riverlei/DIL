...
echo "Updating profit sheet..."
cd "$DIL_ROOT"/crawl/crawl
if [ "$3" == "" ];then
    echo "Scraping profit sheet data..."
    scrapy crawl "$spider" -a year="$1" -a month="$2"
    echo "Scraping profit sheet data done."

    echo "Checking and loading profit sheet data to DB..."

    schema_lst="$DIL_ROOT"/feeds/fdmt_profit_sheet_initial/schemas/000001.lst
    jqf="$DIL_ROOT"/tmp/extract_profit_sheet_bank.jq
    "$DIL_ROOT"/sh/print_regular_report_delta_jq_fields.sh -e ps bank > "$jqf"
    table=securities_profit_sheet_bank
    for code in $(cat "$schema_lst" <("$DIL_ROOT"/sh/find_regular_report_not_scraped_delta.sh ps "$1" "$2") | sort | uniq -d)
    do
    ...
    schema_lst="$DIL_ROOT"/feeds/fdmt_profit_sheet_initial/schemas/000002.lst
    jqf="$DIL_ROOT"/tmp/extract_profit_sheet_general.jq
    "$DIL_ROOT"/sh/print_regular_report_delta_jq_fields.sh -e ps general > "$jqf"
    ...
    schema_lst="$DIL_ROOT"/feeds/fdmt_profit_sheet_initial/schemas/601318.lst
    jqf="$DIL_ROOT"/tmp/extract_profit_sheet_insurance.jq
    "$DIL_ROOT"/sh/print_regular_report_delta_jq_fields.sh -e ps insurance > "$jqf"
    ...
    schema_lst="$DIL_ROOT"/feeds/fdmt_profit_sheet_initial/schemas/000686.lst
    jqf="$DIL_ROOT"/tmp/extract_profit_sheet_securities.jq
    "$DIL_ROOT"/sh/print_regular_report_delta_jq_fields.sh -e ps securities > "$jqf"
    ...
    echo "Checking and loading profit sheet data to DB done."
else
    echo "Scraping profit sheet data..."
