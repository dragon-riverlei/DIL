...
usage(){
    echo "usage:"
    echo "check_profit_sheet_insurance.sh <file_list>"
    exit 1
}
...
jqf="$DIL_ROOT"/tmp/check_profit_sheet_insurance.jq
rm -rf "$jqf"

"$DIL_ROOT"/sh/print_regular_report_delta_jq_fields.sh -c ps insurance > "$jqf"
