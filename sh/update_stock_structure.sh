...
codes=$(psql dil -t -c "select case when sc.market = 'Shanghai' then 'sh' || kpi3.code else 'sz' || kpi3.code end from (select code from check_securities_kpi_c3_cap()) kpi3 join securities_code sc on sc.code = kpi3.code;" | awk '{print $1}' | tr '\n' ',')
